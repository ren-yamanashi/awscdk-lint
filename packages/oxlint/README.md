...
There are two ways to configure oxlint with this plugin.

### `oxlint.config.ts` (TypeScript config with preset)
...
### `.oxlintrc.json` (JSON config)

The following enables the same rule set as the `recommended` preset:
