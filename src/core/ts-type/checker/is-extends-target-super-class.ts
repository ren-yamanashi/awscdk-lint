...
export const isExtendsFromTargetSuperClassOxlint = (
  ...
  const rawTypeName = safeCall(() => checker.typeToString(type), "");
  ...
  if (rawTypeName.startsWith("typeof ")) {
    const signatures = safeCall(() => checker.getSignaturesOfType(type, 1), []);
    for (const sig of signatures) {
      const returnType = safeCall(() => checker.getReturnTypeOfSignature(sig), undefined);
      if (
        returnType &&
